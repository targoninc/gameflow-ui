(function(global) {
    ...
    var LiteGraph = (global.LiteGraph = {
        ...
        MAX_NUMBER_OF_NODES: 1000, // avoid infinite loops, TODO: proper infinite loop detection
        ...
        auto_load_slot_types: false, // [if want false, use true, run, get vars values to be statically set, then disable]
        // nodes types and nodeclass association with node types need to be calculated,
        // if you dont want this, calculate once and set registered_slot_[in/out]_types and slot_types_[in/out]
		...
		do_add_triggers_slots: false, // [true!] will create and connect event slots when using action/events connections,
        ...
        registerNodeType: function(type, base_class) {
            ...
            // TODO: one would want to know input and output :: this would allow trought registerNodeAndSlotType to get all the slots types
        ...
        registerNodeAndSlotType: function(type, slot_type, out = false){
            let base_class;
            if (type.constructor === String && this.registered_node_types[type] !== "anonymous") {
                base_class = this.registered_node_types[type];
            } else {
                base_class = type;
            }
            ...
            for (let i = 0; i < aTypes.length; ++i) {
                ...
                this[registerTo][sT].nodes.push(base_class.constructor.type);
        ...
        wrapFunctionAsNode: function(
            ...
            // TODO: generating code like this is cursed, there must be a better way
         ...
         * @param {Object} options to set an extra array of options on the node object
         */
        ...
        reloadScripts: function(folder_wildcard) {
         ...
         * @param {Number|String} type_a
         * @param {Number|String} type_b
    ...
    LGraph.prototype.save = function() {
        return this._nodes;
    }
    ...
    LGraph.prototype.getGroups = function() {
        return this._groups;
    }
     ...
     * @param {Number} mode
     */
    LGraph.prototype.sendEventToAllNodes = function(eventname, params, mode = LiteGraph.ALWAYS) {
    ...
    LGraph.prototype.remove = function(node) {
		...
		this.beforeChange(); // sure? - almost sure is wrong // TODO: figure out if wrong
        ...
        this.setDirtyCanvas(true, true);
		this.afterChange(); // sure? - almost sure is wrong // TODO: figure out if wrong
        this.change();
     ...
     * @param {Class} classObject the class itself (not a string)
     * @param {Array} result
    ...
    LGraph.prototype.serialize = function() {
        ...
        for (let i in this.links) {
            ...
            if (!link.serialize) {
                // weird bug I haven't solved yet, TODO: solve :)
                console.warn(
     ...
     * @method configure
     * @param {String|Object} data configure a graph from a JSON string
    ...
    */
    ...
    /**
     * App Class for all the node type classes
     * @class LGraphNode
    ...
    LGraphNode.prototype.setOutputData = function(slot, data) {
        ...
        // this may be slow and a niche case
        /*
     ...
     */
    LGraphNode.prototype.addInput = function(name, type = 0, extra_info = false) {
    ...
    LGraphNode.prototype.computeSize = function(out) {
        ...
        function compute_text_size(text) {
            ...
            return font_size * text.length * 0.75;
        }
    ...
    */
     ...
     * @return {boolean}
     */
    LGraphNode.prototype.isPointInside = function(x, y, margin = 0, skip_title) {
...
*/
    ...
    function LGraphCanvas(canvas, graph, options = {}) {
        ...
        this.live_mode = false; // TODO: figure out what this does
        this.show_info = true;
	...
	*/
     ...
     */
    LGraphCanvas.prototype.setDirty = function(fgcanvas, bgcanvas = false) {
        if (fgcanvas) {
            this.dirty_canvas = true;
        }
        if (bgcanvas) {
	...
	*/
		...
		else if (this.selected_group && !this.read_only)
		{
            ...
            if (node) {
                ...
                if (this.connecting_node) {
                    ...
                    let nodeOverInput = this.isOverNodeInput(node, e.canvasX, e.canvasY, pos);
                    let nodeOverOutput = this.isOverNodeOutput(node, e.canvasX, e.canvasY, pos);
                    if (nodeOverInput !== -1 || nodeOverOutput !== -1) {
                        this.canvas.style.cursor = "crosshair";
                    }
                }
                ...
                // if over widget
                if (node.widgets) {
                    if (this.isOverNodeWidget(node, e.canvasX, e.canvasY, pos) !== -1) {
			...
			// send event to node if capturing input (used with widgets that allow drag outside the area of the node)
            ...
            if (this.node_dragged && !this.live_mode) {
				// console.log("dragging!",this.selected_nodes);
    ...
    LGraphCanvas.prototype.isOverNodeWidget = function(
        node,
        canvasx,
        canvasy,
        slot_pos
	...
	*/
    ...
    LGraphCanvas.prototype.drawNode = function(node, ctx) {
        ...
        if (!node.flags.collapsed) {
            ...
            if (node.widgets) {
                ...
                const widget_active = this.node_widget && this.node_widget[0] === node
                    ? this.node_widget[1]
                    : null;
                this.drawNodeWidgets(node, widgets_y, ctx, widget_active);
            }
    ...
    LGraphCanvas.prototype.drawNodeShape = function(node, ctx, size, fgcolor, bgcolor, selected, mouse_over) {
        ...
        const old_alpha = ctx.globalAlpha; // maybe not necessary
        ...
        if (render_title || title_mode === LiteGraph.TRANSPARENT_TITLE) {
            const titleVisible = title_mode !== LiteGraph.TRANSPARENT_TITLE && (node.constructor.title_color || this.render_title_colored);
            if (node.onDrawTitleBar) {
                ...
                if (shape === LiteGraph.BOX_SHAPE || low_quality) {
                    ...
                    ctx.roundRect(0, -title_height, size[0] + 1, title_height, [this.round_radius]);
                }
                ctx.fill();
                ctx.shadowColor = "transparent";
            }
			...
			if (!node.flags.collapsed && node.subgraph && !node.skip_subgraph_button) {
				const nTH = LiteGraph.NODE_TITLE_HEIGHT;
				const x = node.size[0] - nTH;
				const over = LiteGraph.isInsideRectangle( this.graph_mouse[0] - node.pos[0], this.graph_mouse[1] - node.pos[1], x+2, -nTH+2, nTH-4, nTH-4 );
				ctx.fillStyle = over ? "#888" : "#555";
				if( shape === LiteGraph.BOX_SHAPE || low_quality)
					ctx.fillRect(x+2, -nTH+2, nTH-4, nTH-4);
				else
				{
					ctx.beginPath();
					ctx.roundRect(x+2, -nTH+2, nTH-4, nTH-4,[4]);
					ctx.fill();
				}
				ctx.fillStyle = "#333";
				ctx.beginPath();
				ctx.moveTo(x + nTH * 0.2, -nTH * 0.6);
				ctx.lineTo(x + nTH * 0.8, -nTH * 0.6);
				ctx.lineTo(x + nTH * 0.5, -nTH * 0.3);
				ctx.fill();
			}
        ...
        if (selected) {
            this.drawSelectionBox(node, ctx, size, fgcolor, area, shape, title_mode, title_height);
        }
    ...
    /**
     * draws a selection box around a node (used when a node is selected)
     * @method drawNodeShape
     **/
    LGraphCanvas.prototype.drawSelectionBox = function(
        node,
        ctx,
        size,
        fgcolor,
        ...
        const SELECTION_DISTANCE = 0;
        const RADIUS_INCREASE = 1;

        if (shape === LiteGraph.BOX_SHAPE) {
            ctx.rect(
                area[0] - SELECTION_DISTANCE,
                area[1] - SELECTION_DISTANCE,
                area[2] + (SELECTION_DISTANCE * 2),
                area[3] + (SELECTION_DISTANCE * 2)
            );
        } else if (
            shape === LiteGraph.ROUND_SHAPE ||
            ...
            ctx.roundRect(
                ...
                area[2] + (SELECTION_DISTANCE * 2),
                area[3] + (SELECTION_DISTANCE * 2),
                [this.round_radius + RADIUS_INCREASE]
            );
    ...
    LGraphCanvas.prototype.drawConnections = function(ctx) {
        ...
        for (const node of nodes) {
            ...
            for (let i = 0; i < node.inputs.length; i++) {
                ...
                // skip links outside the visible area of the canvas
                ...
                this.renderLink(ctx, start_node_slotpos, end_node_slotpos, link, false, false, null, start_dir, end_dir);
     ...
     * @param {boolean|number} flow show flow animation (for events)
     * @param {string|null} color the color for the link
    ...
    LGraphCanvas.prototype.renderLink = function(
        ...
        for (let i = 0; i < num_sublines; i += 1) {
            ...
            let start_offset_x, start_offset_y, end_offset_x, end_offset_y;

            switch (this.links_render_mode) {
                case LiteGraph.SPLINE_LINK:
                    ctx.moveTo(a[0], a[1] + offsety);

                    function mapDirToSplineOffset(dir) {
                        let offset = {x: 0, y: 0};
                        switch (dir) {
                            case LiteGraph.LEFT:
                                offset.x = dist * -0.25;
                    ...
                    start_offset_x = mapDirToSplineOffset(start_dir).x;
                    start_offset_y = mapDirToSplineOffset(start_dir).y;
                    end_offset_x = mapDirToSplineOffset(end_dir).x;
                    end_offset_y = mapDirToSplineOffset(end_dir).y;
                    ...
                    break;
                case LiteGraph.LINEAR_LINK:
                    ctx.moveTo(a[0], a[1] + offsety);

                    function mapDirToLinearOffset(dir) {
                        let offset = {x: 0, y: 0};
                        switch (dir) {
                            case LiteGraph.LEFT:
                                offset.x = -1;
                    ...
                    start_offset_x = mapDirToLinearOffset(start_dir).x;
                    start_offset_y = mapDirToLinearOffset(start_dir).y;
                    end_offset_x = mapDirToLinearOffset(end_dir).x;
                    end_offset_y = mapDirToLinearOffset(end_dir).y;
            ...
            end_dir !== LiteGraph.CENTER)
        {
            ...
            // render arrow, TODO: fix so it actually displays them properly
    ...
    // TODO: figure out algorithm for curved connections
    ...
    LGraphCanvas.prototype.drawExecutionOrder = function(ctx) {
        ...
        for (let i = 0; i < this.visible_nodes.length; i++) {
            let node = this.visible_nodes[i];
            ctx.fillStyle = "black";
            ctx.fillRect(
     ...
     * @param node The node owning the widget
     * @param ctx The canvas context to render on
     * @param posY The position on the y-axis in the canvas where to draw the widget
     * @param width The base width of the widget
     * @param margin The margin of the widget within the node
     * @param show_text If the text of the widget should be shown
     * @param active_widget If the widget is active
     * @return The posY for the next widget
     */
    LGraphCanvas.prototype.drawNodeWidget = function(w, node, ctx, posY, width, margin, show_text, active_widget) {
        let y = posY;
        if (w.y) {
            y = w.y;
        }
        ...
        w.pos = [margin, y];
        w.size = [widget_width, LiteGraph.NODE_WIDGET_HEIGHT];
        w.last_y = y;
        ...
        switch (w.type) {
            case "button":
                ...
                ctx.fillRect(margin, y, widget_width - margin * 2, widgetHeight);
                ...
                if (!w.disabled) {
                    ctx.strokeRect(margin, y, widget_width - margin * 2, widgetHeight);
                } else {
                    ctx.textAlign = "center";
                    ctx.fillStyle = LiteGraph.WIDGET_TEXT_COLOR;
                    ctx.fillText(w.name, widget_width * 0.5, y + widgetHeight * 0.7);
                }
                break;
            case "toggle":
                ctx.textAlign = "left";
                ...
                ctx.beginPath();
                if (show_text) {
                    ctx.roundRect(margin, y, widget_width - margin * 2, widgetHeight, [widgetHeight * 0.5]);
                } else {
                    ctx.rect(margin, y, widget_width - margin * 2, widgetHeight );
                }
                ctx.fill();
                ...
                ctx.beginPath();
                ctx.arc( widget_width - margin * 2, y + widgetHeight * 0.5, widgetHeight * 0.36, 0, Math.PI * 2 );
                ctx.fill();
                if (show_text) {
                    ctx.fillStyle = LiteGraph.WIDGET_SECONDARY_TEXT_COLOR;
                    if (w.name != null) {
                        ctx.fillText(w.name, margin * 2, y + widgetHeight * 0.7);
                    }
                    ctx.fillStyle = w.value ? LiteGraph.WIDGET_TEXT_COLOR : LiteGraph.WIDGET_SECONDARY_TEXT_COLOR;
                    ctx.textAlign = "right";
                    ctx.fillText(
                        w.value
                            ? w.options.on || "true"
            ...
            case "slider":
                ...
                ctx.fillRect(margin, y, nvalue * (widget_width - margin * 2), widgetHeight);
                if(show_text && !w.disabled)
                    ctx.strokeRect(margin, y, widget_width - margin * 2, widgetHeight);
                if (w.marker) {
                    const marker_nvalue = (w.marker - w.options.min) / range;
                    ctx.fillStyle = "#AA9";
                    ctx.fillRect( margin + marker_nvalue * (widget_width - margin * 2), y, 2, widgetHeight );
                }
                if (show_text) {
                    ctx.textAlign = "center";
                    ctx.fillStyle = LiteGraph.WIDGET_TEXT_COLOR;
            ...
            case "combo":
                ...
                if (show_text) {
                    ...
                    if(!w.disabled)
                    {
                        ...
                        ctx.lineTo(margin + 6, y + widgetHeight * 0.5);
                        ctx.lineTo(margin + 16, y + widgetHeight - 5);
                        ctx.fill();
                        ctx.beginPath();
                        ctx.moveTo(widget_width - margin - 16, y + 5);
                        ctx.lineTo(widget_width - margin - 6, y + widgetHeight * 0.5);
                        ctx.lineTo(widget_width - margin - 16, y + widgetHeight - 5);
                        ctx.fill();
                    }
                    ctx.fillStyle = LiteGraph.WIDGET_SECONDARY_TEXT_COLOR;
                    ctx.fillText(w.name, margin * 2 + 5, y + widgetHeight * 0.7);
                    ctx.fillStyle = LiteGraph.WIDGET_TEXT_COLOR;
                    ctx.textAlign = "right";
                    if (w.type === "number") {
                        let numDisplay;
                        if (w.options.precision !== undefined) {
                            numDisplay = Number(w.value).toFixed(w.options.precision);
                        } else {
                            numDisplay = Number(w.value).toString();
                        }
                        ctx.fillText(numDisplay, widget_width - margin * 2 - 20, y + widgetHeight * 0.7);
                    } else {
                        let v = w.value;
                        if (w.options.values) {
                        ...
                        ctx.fillText(v, widget_width - margin * 2 - 20, y + widgetHeight * 0.7);
                    }
                }
                break;
            case "string":
            case "text":
                ...
                if (show_text) {
                    ...
                    ctx.fillStyle = LiteGraph.WIDGET_TEXT_COLOR;
                    ctx.textAlign = "right";
                    ctx.fillText(String(w.value).substr(0,30), widget_width - margin * 2, y + widgetHeight * 0.7); //30 chars max
                    ctx.restore();
                }
                break;
            default:
                if (w.draw) {
                    w.draw(ctx, node, widget_width, y, widgetHeight);
                }
                break;
        }
        posY += (w.computeSize ? w.computeSize(widget_width)[1] : widgetHeight) + 4;
        ctx.globalAlpha = this.editor_alpha;
        return posY;
    }
    ...
    LGraphCanvas.prototype.drawNodeWidgets = function(
        ...
        for (let i = 0; i < widgets.length; i++) {
            let w = widgets[i];
            posY = this.drawNodeWidget(w, node, ctx, posY, width, margin, show_text, active_widget);
        }
        ctx.restore();
		ctx.textAlign = "left";
    };
			...
			switch (w.type) {
				...
				case "combo":
					if (event.type === LiteGraph.pointerevents_method+"move" && w.type === "number") {
                        ...
                        const delta = this.getDelta(x, widget_width, 40);
						if (w.type === "number") {
    ...
    /**
     * Gets a "delta" of the direction arrow on a widget<br>
     * (←  widget  →)<br>
     * (-1)--(0)--(1)
     * @param x {number} the x position of the mouse
     * @param width {number} the width of the widget
     * @param padding {number} the width of the arrows
     * @returns {number} the delta (-1, 0, 1)
     */
    LGraphCanvas.prototype.getDelta = function(x, width, padding) {
        if (x < padding) {
            return -1;
     ...
     * this feature was designed when graphs were meant to create user interfaces
    ...
    LGraphCanvas.onShowMenuNodeProperties = function(
        ...
        for (var i in node.properties) {
            ...
            let subtype = value === "&lt;computed&gt;" ? "computed" : "";
            entries.push({
                content:
                    "<span class='property_name'>" +
                    (info.label ? info.label : i) +
                    "</span>"
                    + '<span class="property_value_display ' + subtype + '">' +
                    value +
                    "</span>",
                value: i
            });
        ...
        function inner_clicked(v, options, e, prev) {
            ...
            if (node.properties[v.value] === "<computed>") {
                return;
            }
    ...
    LGraphCanvas.onShowPropertyEditor = function(item, options, e, menu, node) {
        ...
        // TODO refactor :: use createDialog ? yes? fucker!
        
        var dialog = document.createElement("div");
        dialog.is_modified = false;
        dialog.classList.add("graphdialog");
        let extraClasses = ["propertyEditor"];
        ...
        if (input) {
            ...
            input.addEventListener("keydown", function(e) {
                ...
                if (e.code === "Escape") {
                    ...
                    inner(); // save
                } else if (e.code !== "Enter" && e.target.localName !== "textarea") {
                    return;
                }
                e.preventDefault();
                e.stopPropagation();
    ...
    // TODO: refactor: there are different dialogs, some uses createDialog some don't
    LGraphCanvas.prototype.prompt = function(title, value, callback, event, multiline) {
        ...
        input.addEventListener("keydown", function(e) {
            ...
            if (e.code === "Escape") {
                dialog.close();
            } else if (e.keyCode === "Enter" && e.target.localName !== "textarea") {
                if (callback) {
                    callback(this.value);
                }
                dialog.close();
    ...
    LGraphCanvas.prototype.showSearchBox = function(event, options) {
        ...
        dialog.innerHTML = "<div class='draggable flex search_header'><span class='name'>Search</span> <input autofocus type='text' class='value rounded'/></div>";
    ...
    // TODO: refactor, there are different dialogues, some uses createDialog, some don't
    LGraphCanvas.prototype.createDialog = function(html, options, extraClasses = []) {
	...
	LGraphCanvas.prototype.createPanel = function(title, options) {
		...
		root.addWidget = function(type, name, value, options, callback) {
			...
			else if (type === "string" || type === "number")
			{
                ...
                value_element.innerText = LGraphCanvas.getPropertyPrintableValue(value, options.values);
    ...
    LGraphCanvas.prototype.closePanels = function(){
        let panel = document.querySelector("#node-panel") ?? document.querySelector("#option-panel");
		if (panel) {
            panel.close();
        }
    }
    ...
    if (typeof(window) != "undefined" && window.CanvasRenderingContext2D && !window.CanvasRenderingContext2D.prototype.roundRect) {
		...
		// make it compatible with native radius type
    ...
    /**
     *
     * @param name {String} name of the item to search an icon for
     * @returns {String} Html of the icon
     */
    ContextMenu.prototype.getIcon = function(name) {
        const google_icon_mappings = {
            "Properties": "build",
            "Title": "edit_square",
            "Mode": "settings",
            "Resize": "photo_size_select_small",
            "Collapse": "unfold_less",
            "Outputs": "output",
            "Inputs": "input",
            "Pin": "push_pin",
            "Color": "format_color_fill",
            "Colors": "format_color_fill",
            "Font size": "format_size",
            "Shapes": "rounded_corner",
            "Clone": "content_copy",
            "Remove": "delete_forever"
        };
        if (google_icon_mappings[name] !== undefined) {
            return '<span class="material-icons md-24">' + google_icon_mappings[name] + '</span>';
        } else {
            return "";
        }
    }
    ...
    ContextMenu.prototype.addItem = function(name, value, options) {
        ...
        if (value === null) {
            ...
            let icon = this.getIcon(element.innerHTML);
            editable = !(value.submenu || value.has_submenu || value.disabled || value.callback !== undefined);
            element.innerHTML = icon + "<span class='flex text-spacer'>" + element.innerHTML + "</span>";
            element.value = value;
